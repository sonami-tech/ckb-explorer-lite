...
/**
 * Format a Unix timestamp to a short relative time string (e.g., "1d", "3h").
 * Used for compact mobile displays.
 */
export function formatRelativeTimeShort(timestamp: bigint | number): string {
	const ts = typeof timestamp === 'bigint' ? Number(timestamp) : timestamp;
	const now = Date.now();
	const diff = now - ts;
